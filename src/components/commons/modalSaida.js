...
export default function ModalSaida({ isOpen, onClose, onAdd }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/add-saida", {
        method: "POST",
      ...
      const newSaida = await response.json();
      if (newSaida) {
        onAdd(newSaida); 
        onClose(); 
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-96">
        <h2 className="text-xl font-bold mb-4">Adicionar Saída</h2>
